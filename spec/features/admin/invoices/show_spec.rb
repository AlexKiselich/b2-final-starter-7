...
describe "Admin Invoices Index Page" do
  before :each do
    ...
    @coupon_1 = Coupon.create!(name: "$10 Off", code: "10DO", amount: 10, discount: 0, status: 1, merchant: @m1)

    @c1 = Customer.create!(first_name: "Yo", last_name: "Yoz", address: "123 Heyyo", city: "Whoville", state: "CO", zip: 12345)
    @c2 = Customer.create!(first_name: "Hey", last_name: "Heyz", address: "123 Heyyo", city: "Whoville", state: "CO", zip: 12345)

    @i1 = Invoice.create!(customer_id: @c1.id, status: 2, created_at: "2012-03-25 09:54:09")
    @i2 = Invoice.create!(customer_id: @c2.id, status: 1, created_at: "2012-03-25 09:30:09", coupon: @coupon_1)
    ...
    @ii_3 = InvoiceItem.create!(invoice_id: @i2.id, item_id: @item_2.id, quantity: 44, unit_price: 12, status: 2)
    @ii_4 = InvoiceItem.create!(invoice_id: @i2.id, item_id: @item_2.id, quantity: 6, unit_price: 12, status: 2)
  ...
  it "displays the name and code of the coupon that was used and I see subtotal and grand total revenue, when a coupon is used" do
    visit admin_invoice_path(@i2)

    expect(page).to have_content("Subtotal Revenue: $#{@i2.total_revenue}")
    expect(page).to have_content("Grand Total Revenue: $#{@i2.grand_total}")
